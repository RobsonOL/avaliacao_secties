...
dim_discentes <- discentes_pb |> 
  dplyr::filter(ANO >= 2017, ANO <= 2020) |> 
  dplyr::group_by(
...
df_discentes <- dim_discentes |> 
  ...
  dplyr::left_join(discentes_pb |> 
                     dplyr::filter(ANO >= 2017, ANO <= 2020) |> # FAPESQ só começou a ser informado em 2017
...
df_bolsas <- bolsas_pb |> 
  ...
  dplyr::reframe(QT_BOLSA_TOTAL = sum(QT_BOLSA_ANO),
                   VL_BOLSA_TOTAL = sum(VL_BOLSA_ANO),
                   QT_BOLSA_FAPESQ = ifelse(SG_PROGRAMA_CAPES == "FAPESQ",
                                            sum(QT_BOLSA_ANO), 0),
                   VL_BOLSA_FAPESQ = ifelse(SG_PROGRAMA_CAPES == "FAPESQ",
                                            sum(VL_BOLSA_ANO), 0),
                   QT_BOLSA_NAO_FAPESQ = ifelse(SG_PROGRAMA_CAPES != "FAPESQ",
                                                sum(QT_BOLSA_ANO), 0),
                   VL_BOLSA_NAO_FAPESQ = ifelse(SG_PROGRAMA_CAPES != "FAPESQ",
                                                sum(VL_BOLSA_ANO), 0)) |> 
  dplyr::ungroup() |> 
  dplyr::mutate(BOLSA_APENAS_FAPESQ = if_else(QT_BOLSA_FAPESQ > 0 & QT_BOLSA_NAO_FAPESQ == 0, 1, 0)) |>
  dplyr::mutate(BOLSA_MUDOU = if_else(QT_BOLSA_FAPESQ > 0 & QT_BOLSA_NAO_FAPESQ > 0, 1, 0)) |> 
...
df_discentes_bolsa <- df_discentes |>
  ...
  dplyr::left_join(df_bolsas, by = c("ID_PESSOA", "NM_DISCENTE",
...
df_discentes_bolsa_tese <- df_discentes_bolsa |> 
  dplyr::left_join(
    teses_pb |> 
      dplyr::select(ID_PESSOA, NM_PRODUCAO, NM_GRAU_ACADEMICO,
